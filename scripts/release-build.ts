...
export type ReleaseBuildOptions = { readonly outputDirectory?: string; readonly releaseVersion?: string; readonly sourceCommit?: string };
...
export async function buildLinuxRelease(options: ReleaseBuildOptions = {}): Promise<{ readonly directory: string; readonly version: string }> {
  ...
  try {
    ...
    await Promise.all([writeFile(join(temporary, "manifest.json"), serializeManifest(manifest)), writeFile(join(temporary, "SHA256SUMS"), serializeChecksums(manifest)), writeFile(join(root, "install.sh"), await renderInstaller(manifest))]);
    await verifyLinuxRelease(temporary);
...
if (import.meta.main) {
  const [flag, releaseVersion] = process.argv.slice(2);
  if ((flag !== undefined && flag !== "--release-version") || (flag === "--release-version" && releaseVersion === undefined) || process.argv.length > 4) {
    process.stderr.write("usage: bun run release:build [--release-version vX.Y.Z]\n");
    process.exitCode = 1;
  } else {
    buildLinuxRelease({ releaseVersion }).then((release) => process.stdout.write(`Built ${release.directory} (${release.version})\n`)).catch((error: unknown) => {
