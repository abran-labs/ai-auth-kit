...
import { AuthPolicySchema, getAuthPolicy, type AuthPolicy } from "./auth-policy-registry.js";
import { CatalogProvenanceSchema, ModelsDevSourceSchema, type ModelsDevSource } from "./catalog-source-schema.js";
...
type NormalizedProvider = z.output<typeof NormalizedProviderSchema>;
...
function normalizeModel(model: ModelsDevSource[string]["models"][string]): z.output<typeof NormalizedModelSchema> {
  return {
    id: model.id,
...
function normalizeProvider(provider: ModelsDevSource[string]): NormalizedProvider {
  const envNames = [...provider.env].sort(compareStrings);
  const authPolicy: AuthPolicy = getAuthPolicy(provider.id, envNames);
  return {
    id: provider.id,
...
export function normalizeModelsDevCatalog(source: unknown, provenance: unknown): NormalizedCatalog {
