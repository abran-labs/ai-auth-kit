...
import { fetchModelsDevCatalog, type CatalogFetch } from "./catalog-http.js";
import { normalizeModelsDevCatalog, type NormalizedCatalog } from "./catalog-normalize.js";

const MODELS_DEV_URL = "https://models.dev/api.json";
const REFRESH_GUARD_MS = 5 * 60 * 1000;
const HOUR_MS = 60 * 60 * 1000;

export type CatalogRefreshResult = { readonly source: "network" | "cache" | "snapshot"; readonly catalog: NormalizedCatalog };
...
export type CatalogRefreshTimer = { readonly dispose: () => void };
...
export function createCatalogRefresher(options: CatalogRefresherOptions): { readonly refresh: () => Promise<CatalogRefreshResult>; readonly startHourlyRefresh: () => CatalogRefreshTimer } {
  ...
  let lastAttempt = 0;
  let inFlight: Promise<CatalogRefreshResult> | undefined;

  const fallback = async (): Promise<CatalogRefreshResult> => {
  ...
  const refresh = async (): Promise<CatalogRefreshResult> => {
    ...
    inFlight = (async () => {
      ...
      try {
        ...
        if (response.kind === "not-modified" && existing !== null) return { source: "cache", catalog: existing.catalog };
        if (response.kind === "not-modified") return fallback();
