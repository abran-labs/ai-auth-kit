...
export class SafeDirectory {
    ...
    static async openStorage(configDir, managedRoot) {
        ...
        try {
            for (const component of components(target)) {
                ...
                try {
                    ...
                    if (managed)
                        throw new Error("Storage managed directory must be an owned, non-writable real directory", { cause: error });
                    throw error;
                }
                ...
                if (managed)
                    await next.handle.chmod(DIRECTORY_MODE);
                await current.close();
                current = next;
...
//# sourceMappingURL=safe-dir.js.map