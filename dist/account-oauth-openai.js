...
import { extractOpenAiAccountId, now, OPENAI_CLIENT_ID, OPENAI_CODEX_BASE_URL, OPENAI_ISSUER, readJson, requireFetch, secretRef, sleep } from "./account-oauth-shared.js";
export async function refreshOpenAiCodexToken(refreshToken) {
    ...
    const token = await readJson(response, "OpenAI token refresh");
...
async function pollDeviceAuthorization(deviceAuthId, userCode, intervalMs, deps) {
    ...
    while (true) {
        ...
        if (response.ok)
            return readJson(response, "OpenAI device authorization polling");
        if (response.status !== 403 && response.status !== 404)
            throw new Error(`OpenAI device login failed: ${response.status} ${await response.text()}`);
    }
}
async function loginWithDeviceCode(prompts, deps) {
    const fetchImpl = requireFetch(deps);
    const device = await readJson(await fetchImpl(`${OPENAI_ISSUER}/api/accounts/deviceauth/usercode`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ client_id: OPENAI_CLIENT_ID }) }), "OpenAI device login start");
    ...
    return readJson(response, "OpenAI token exchange");
}
export async function loginOpenAiAccount(kit, provider, prompts, deps = {}) {
    let token;
    try {
        token = await loginOpenAiAccountInBrowser(prompts, deps);
    }
    catch (error) {
        prompts.info(`Browser sign-in unavailable. Falling back to device code. ${error instanceof Error ? error.message : String(error)}`);
        token = await loginWithDeviceCode(prompts, deps);
    }
...
//# sourceMappingURL=account-oauth-openai.js.map