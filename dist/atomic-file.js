...
function owned(stat) {
    ...
    if (!getuid)
        throw new Error("Storage ownership checks require Linux process.getuid");
    return stat.uid === getuid();
}
async function checkedFile(directory, name) {
    try {
...
function fault(boundary) {
    if (process.env.AI_AUTH_KIT_INTERRUPT_AT === boundary)
        throw new Error(`Interrupted atomic write after ${boundary}`);
}
export async function readFileAnchored(directory, name) {
...
export async function writeFileAtomic(directory, name, value) {
    ...
    try {
        ...
        if (!committed)
            throw new Error("Atomic write target disappeared before commit validation");
        try {
            const committedStat = await committed.stat();
            if (committedStat.dev !== tempStat.dev || committedStat.ino !== tempStat.ino)
                throw new Error("Atomic write target inode changed during commit");
        }
        finally {
        ...
        await handle.close();
        if (temporary)
            await rm(`${directory.path()}/${temp}`, { force: true });
    }
}
export async function unlinkFileAnchored(directory, name) {
    const target = await checkedFile(directory, name);
    if (!target)
        return;
    await target.close();
    if (process.env.AI_AUTH_KIT_INTERRUPT_AT === "unlink")
        throw new Error("Interrupted atomic write after unlink");
